...
if [[ -z "${PEER_ID:-}" ]]; then
    ...
    info ""
    info "Once running, get the PeerId from the health endpoint and update"
    info "hm-net/bootstrap.toml with the bootstrap address:"
    info "  peers = [\"/ip4/${STATIC_IP}/tcp/4256/p2p/<PEER_ID>\"]"
    exit 0
fi
...
info ""
info "  To make this the default for all users, update"
info "  hm-net/bootstrap.toml:"
info ""
info "    peers = [\"$BOOTSTRAP_ADDR\"]"
info ""
