...
in which every well-typed term is indexed by its SHA-256 hash, with all dependencies
...
at scale: many agents and mathematicians contributing simultaneously, building on each
...
\begin{quote}
\emph{Inspired by Bitcoin's insight that trust can emerge from verification rather than
authority~\cite{nakamoto2008}, HashMath applies content-addressing to formal mathematics:
naming conflicts are resolved without trusted curators.}
\end{quote}

\paragraph{Paper outline.}
Section~\ref{sec:cic} specifies the HashMath CIC dialect---term constructors, typing rules,
and reductions.
Section~\ref{sec:content} defines the content-addressing scheme and hash functions.
Section~\ref{sec:trust} analyzes verification and trust properties.
Section~\ref{sec:architecture} describes the knowledge base architecture, including the
type-to-inhabitants index and interoperability with existing proof assistants.
Section~\ref{sec:collaboration} discusses AI and human collaboration at scale.
Section~\ref{sec:challenges} addresses challenges and limitations.
Section~\ref{sec:related} surveys related work.
Section~\ref{sec:prototype} describes the reference implementation's distribution prototype.
Section~\ref{sec:roadmap} presents a phased development roadmap, and
Section~\ref{sec:indices} outlines future work on advanced indices.
...
\end{tikzpicture}
\caption{Formalized mathematics today exists in isolated silos with duplicated effort
and incompatible naming.
HashMath provides a single content-addressed pool where every unique proof term has
...
HashMath retains 8 of these (those that carry type-theoretic content) and excludes the
4 that serve only elaboration, metaprogramming, or syntactic sugar.
Two new constructors are added: $\textsf{iref}$ for block-relative references in mutual
inductive definitions, and $\hrefE$ for hash-referenced subterms in the storage layer
(Section~\ref{sec:subterm-hashing}), for 10 constructors total.

\begin{definition}[HashMath expression grammar]
\label{def:expr}
HashMath terms are defined by the following grammar, parameterized by a type $\alpha$:
\[
  e \;\;::=\;\;
    \begin{aligned}[t]
      &\bvar(n) \\
...
$i \in \mathbb{N}$, $k \in \mathbb{N}$ is a block-relative type index,
and $a : \alpha$ is a hash reference value whose type is determined by the phantom parameter $\alpha$.

When $\alpha = \bot$ (the empty type), the $\hrefE$ constructor is uninhabitable,
guaranteeing at the type level that kernel expressions contain no hash references.
When $\alpha = \textsf{Hash}$ (32-byte SHA-256 digests), $\hrefE$ carries a content hash
pointing to a stored subterm.
This phantom-type discipline ensures that hash references can never reach the type checker.
\end{definition}

The constructors are:
\begin{itemize}[nosep]
  \item $\bvar(n)$ --- a de Bruijn bound variable with index $n$.
  \item $\sort(u)$ --- a universe sort at level $u$.
        This is the AST constructor; the familiar names $\Prop$ and $\Type(n)$ are
        abbreviations for $\sort(\textsf{zero})$ and $\sort(\textsf{succ}(n))$, respectively.
  ...
  \item $\textsf{iref}(k, \bar{u})$ --- a block-relative reference to the $k$-th type within a mutual inductive block, instantiated at universe levels $\bar{u}$.
        Used inside constructor types before the block hash is known; resolved to $\const$ when the block is registered (Section~\ref{sec:mutual}).
  \item $\hrefE(a)$ --- a hash reference to a stored subterm.
        Present only in the storage/wire format ($\alpha = \textsf{Hash}$); absent from kernel terms ($\alpha = \bot$).
        See Section~\ref{sec:subterm-hashing}.
\end{itemize}
...
---              & Block-relative ref   & \checkmark & \texttt{iref}: resolved to \texttt{const} \\
---              & Hash reference       & \checkmark & \texttt{href}: storage only ($\alpha{=}\bot$ blocks kernel use) \\
\hline
\end{tabular}
\caption{Lean~4 \texttt{Expr} constructors and their status in HashMath.
  Of the 12 Lean constructors, 8 are retained and 4 are
  excluded (elaboration or syntactic sugar). Two new constructors (\texttt{iref} and \texttt{href}) are added,
  for 10 total. The \texttt{href} constructor is parameterized by a phantom type, making it
  uninhabitable in kernel terms.}
...
\begin{itemize}[nosep]
  ...
  \item $\textsf{imax}(u_1, u_2)$ --- the \emph{impredicative maximum}:
        equals $\textsf{zero}$ if $u_2$ evaluates to $\textsf{zero}$,
        and $\max(u_1, u_2)$ otherwise.
        This is essential for typing $\Pi$-types into $\Prop$
        (see Section~\ref{sec:typing}).
  \item $\textsf{param}(n)$ --- a universe parameter variable, referenced by
        de Bruijn index rather than by name, for the same content-addressing
        reasons that binder names are excluded from terms.
\end{itemize}
...
Two additional completeness rules handle cases where $\iota$-reduction is stuck because the
major premise does not reduce to a constructor:

\begin{itemize}[nosep]
  \item \textbf{K-like reduction}: if $I$ is a $\Prop$ type with exactly one constructor
        and zero fields (e.g., $\textsf{True}$, or $\Eq$ at fixed indices), and the
        recursor's major premise does not reduce to a constructor application,
        the unique constructor is synthesized.
        This is sound because $\Prop$ proof irrelevance guarantees exactly one
        constructor value.

  \item \textbf{Structure expansion}: if $I$ is a single-constructor type with fields
        (a structure) and the major premise does not reduce to a constructor
        application, it is expanded to
        \[c(\bar{p},\, \proj(h_I, 0, x),\, \ldots,\, \proj(h_I, n{-}1, x))\]
        where $x$ is the major premise, enabling $\iota$-reduction to proceed.
\end{itemize}
...
\[
  ...
  \}
\]
where $k$ is the number of universe parameters, $p$ is the number of parameters,
$\mathit{isUnsafe}$ marks the block as unsafe (bypassing the kernel's soundness checks,
used in Lean~4 for implementation-internal types that are not intended to appear in proofs),
and each
...
\paragraph{Recursor shape.}
For each inductive type, the kernel generates a \emph{recursor} (elimination principle)
whose type has the form:
\[
  \textsf{rec}_I : \Pi\;\bar{p}.\; \Pi\;(C : I\;\bar{p} \to \Sort(j)).\;
  \bar{m} \to \Pi\;(x : I\;\bar{p}).\; C\;x
\]
where $\bar{p}$ are the type's parameters, $C$ is the \emph{motive}
(specifying what is to be proved or computed for each element of $I$),
$\bar{m}$ are the \emph{minor premises} (one per constructor, handling recursive
arguments via induction hypotheses), and $x$ is the \emph{major premise}
(the value being eliminated).
The universe $j$ of the motive is subject to the large elimination restriction below.
For indexed families (like $\Eq$), the motive additionally abstracts over the index arguments.

\paragraph{Large elimination restriction.}
$\Prop$ is impredicative (one can quantify over all types and remain in $\Prop$)
and enjoys proof irrelevance (all proofs of the same proposition are definitionally equal).
Together, these features require restricting when a $\Prop$ inductive may eliminate
into $\Type$---so-called \emph{large elimination}.

Without the restriction, one could define $\textsf{Wrap}(A : \Sort(u)) : \Prop$ with a
single constructor $\textsf{mk} : A \to \textsf{Wrap}\;A$, extract data via large elimination
($\textsf{unwrap} : \textsf{Wrap}\;A \to A$), and then derive $\textsf{true} \equiv \textsf{false}$
from proof irrelevance ($\textsf{mk}\;\textsf{true} \equiv \textsf{mk}\;\textsf{false}$
since both inhabit the same $\Prop$).

The rule is: a $\Prop$ inductive \emph{allows} large elimination (motive in $\Sort(j)$ for any $j$)
only when every constructor's non-parameter fields have types in $\Prop$.
Otherwise, the recursor's motive is restricted to $\Prop$ (i.e., $j = 0$).
Examples:
\begin{itemize}[nosep]
  \item $\textsf{True}$ (one constructor, zero fields) --- large elimination allowed.
  \item $\Eq$ (one constructor, zero non-parameter fields) --- large elimination allowed.
        This is what makes $\Eq.\textsf{rec}$ useful for rewriting.
  \item $\textsf{And}$ (one constructor, two $\Prop$ fields) --- large elimination allowed.
  \item $\textsf{Or}$ (two constructors) --- large elimination \textbf{blocked}.
        One cannot decide which disjunct holds.
  \item $\textsf{Exists}$ (constructor field in $\Type$) --- large elimination \textbf{blocked}.
        One cannot extract the witness.
\end{itemize}
When the target sort of the inductive type is polymorphic (e.g., $\Sort(u)$ where $u$
could be instantiated to zero), the restriction is applied conservatively: if any
instantiation could place the type in $\Prop$ with a $\Type$-level field,
large elimination is blocked.
In mutual inductive blocks, the restriction is applied block-wide: if any type
in the block requires restricted elimination, all recursors in the block are restricted.
...
\begin{itemize}[nosep]
  \item $\Quot$ --- the quotient type former:
    \[ \Quot : (\alpha : \Sort(u)) \to (\alpha \to \alpha \to \Prop) \to \Sort(u) \]

  \item $\Quot.\textsf{mk}$ --- the quotient constructor:
    \[ \Quot.\textsf{mk} : \{r : \alpha \to \alpha \to \Prop\} \to \alpha \to \Quot\;\alpha\;r \]

  \item $\Quot.\textsf{lift}$ --- lift a function that respects the equivalence relation:
    \[ \Quot.\textsf{lift} : (\alpha \to \beta) \to (\forall\;a\;b,\; r\;a\;b \to f\;a = f\;b) \to \Quot\;\alpha\;r \to \beta \]

  \item $\Quot.\textsf{ind}$ --- the induction principle:
    \[ \Quot.\textsf{ind} : (\forall\;a,\; \varphi\;(\Quot.\textsf{mk}\;r\;a)) \to \forall\;q,\; \varphi\;q \]
\end{itemize}
...
In Lean~4, definitions carry an opacity attribute
that controls when the definitional equality checker may unfold them
(\texttt{@[reducible]}, \texttt{@[semireducible]}, or~\texttt{@[irreducible]}).
This is a useful performance optimization, but it introduces an
elaboration-level concern into the kernel: two terms may or may not be
definitionally equal depending on the transparency setting.
...
All hashing uses SHA-256~\cite{sha256}.
...
\begin{align*}
  ...
  \hash(\textsf{iref}(k, [l_0, \ldots, l_{n-1}])) &= \SHA(\mathtt{0x18} \concat \textsf{leb128}(k) \concat \textsf{leb128}(n) \concat \hash(l_0) \concat \cdots \concat \hash(l_{n-1}))
\end{align*}
...
\begin{definition}[Derived entity hash]
\label{def:hash-derived}
Each inductive block generates derived entities---individual types, constructors, and recursors---whose
hashes are derived from the block hash:
\begin{align*}
  \hash(\textsf{indType}(h_B, i)) &= \SHA(\mathtt{0x30} \concat h_B \concat \textsf{leb128}(i)) \\
  \hash(\textsf{indCtor}(h_B, i, j)) &= \SHA(\mathtt{0x31} \concat h_B \concat \textsf{leb128}(i) \concat \textsf{leb128}(j)) \\
  \hash(\textsf{indRec}(h_B, i)) &= \SHA(\mathtt{0x32} \concat h_B \concat \textsf{leb128}(i))
\end{align*}
where $h_B$ is the block hash, $i$ is the type index within the block, and $j$ is the constructor index within the type.
\end{definition}

\begin{remark}
The tag bytes are chosen so that levels (\texttt{0x00}--\texttt{0x04}),
expressions (\texttt{0x10}--\texttt{0x18}), the storage-layer hash reference (\texttt{0x19}),
declarations (\texttt{0x20}--\texttt{0x23}),
and derived entities (\texttt{0x30}--\texttt{0x32})
occupy disjoint ranges.
This ensures that no term can collide with a level, declaration, or derived entity at the SHA-256 input level,
even before considering structural differences.
The \texttt{0x19} tag is used only in the stored wire format for $\hrefE$ nodes
(Section~\ref{sec:subterm-hashing}); it never appears in canonical serialization or hash computation.
\end{remark}
...
$\lam(\Nat, \bvar(0))$.
Since binder names are excluded from the hash, they produce identical hashes.
\end{example}
...
A term $t$ is \emph{hash-closed} if every constant reference $\const(h, \bar{u})$ occurring
...
\begin{tikzpicture}[
  ...
  \node[box] (hashed) at (4, 0) {
    ...
    {\tiny Identity: \texttt{a7f3...} (SHA-256 of term)}
  };
...
HashMath uses SHA-256~\cite{sha256} rather than SHA3.
...
Unison's analysis of collision probability applies equally to HashMath: with $2^{128}$
security against collision attacks (the birthday bound for a 256-bit hash), the probability of a collision in a library of $2^{40}$
terms (one trillion) is negligible ($\approx 2^{-176}$).


\subsection{Subterm-Level Hash-Consing}
\label{sec:subterm-hashing}

The hashing scheme described above assigns a content hash to each \emph{declaration}.
But CIC proof terms are deeply nested trees that often share subterms---the type
$\Nat \to \Nat$ may appear thousands of times across a library.
At the declaration level, these shared subterms are duplicated in every serialized declaration
that contains them.

HashMath extends the Merkle DAG from declarations down to \emph{subterms}.
When a declaration is prepared for storage or transmission, a \emph{shatter} operation
replaces large subterms with $\hrefE$ nodes carrying the subterm's content hash.
Each replaced subterm is serialized independently and stored as a separate entry
in the DHT, keyed by its hash.

\begin{definition}[Shatter and reassemble]
\label{def:shatter}
Let $\theta \in \mathbb{N}$ be a size threshold (the reference implementation uses $\theta = 33$,
the cost of one $\hrefE$ node: 1 tag byte + 32 hash bytes).
The \emph{shatter} operation $\textsf{shatter}_\theta : \textsf{Expr} \to \textsf{Expr}_{\textsf{Hash}} \times \textsf{SubtermStore}$
traverses a closed expression bottom-up.
For each subterm $e$ whose canonical serialization exceeds $\theta$ bytes:
\begin{enumerate}[nosep]
  \item Compute $h = \hash(e)$ and the canonical byte serialization $b = \textsf{serialize}(e)$.
  \item Store the mapping $h \mapsto b$ in the \textsf{SubtermStore}.
  \item Replace $e$ in the parent with $\hrefE(h)$.
\end{enumerate}
The inverse operation $\textsf{reassemble} : \textsf{Expr}_{\textsf{Hash}} \times \textsf{SubtermStore} \to \textsf{Expr}$
resolves every $\hrefE(h)$ by deserializing the corresponding entry from the store.
\end{definition}

\paragraph{Phantom type safety.}
The expression type is parameterized as $\textsf{Expr}(\alpha)$.
Kernel operations (type inference, definitional equality, reduction) work with
$\textsf{Expr}(\bot)$, where $\bot$ is an empty type.
Since $\hrefE : \alpha \to \textsf{Expr}(\alpha)$ requires an inhabitant of $\alpha$,
and $\bot$ has none, the $\hrefE$ constructor is \emph{statically uninhabitable}
in kernel code---no match arm, no runtime check, no possibility of a hash reference
reaching the type checker.
The storage layer works with $\textsf{Expr}(\textsf{Hash})$, where $\textsf{Hash}$
is the 32-byte digest type.
An embedding $\textsf{Expr}(\bot) \hookrightarrow \textsf{Expr}(\alpha)$ is provided
for safe upcast.

\paragraph{Hash preservation.}
Shattering does not change a term's content hash:
\[\hash(e) \;=\; \hash\bigl(\textsf{reassemble}(\textsf{shatter}_\theta(e))\bigr)\]
because $\textsf{reassemble}$ recovers the original term exactly
and canonical serialization is deterministic.
The stored subterm entries use the \emph{canonical} serialization (no $\hrefE$ nodes),
so each entry is self-contained.

\paragraph{Deduplication.}
Subterm entries are keyed by content hash.
If two declarations share a subterm, both produce the same hash for that subterm,
and the DHT stores it once.
At scale, this provides global subterm compression: the $\Nat \to \Nat$ type, the
recursor for $\textsf{List}$, or a standard proof pattern that recurs across thousands
of declarations is stored \emph{exactly once} in the entire network.

\paragraph{Wire format.}
Stored declarations use a variant serialization in which $\hrefE$ nodes are encoded
with tag \texttt{0x19} followed by 32 raw hash bytes.
All other constructors use the same tags as canonical serialization.
The format is backward-compatible: declarations without $\hrefE$ nodes are valid
in both the canonical and stored formats, so older nodes that predate subterm
hash-consing can still be read without modification.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
    node/.style={draw, rounded corners, font=\small, fill=blue!6, minimum width=1.6cm, align=center},
    hrefnode/.style={draw, rounded corners, font=\small, fill=purple!12, minimum width=1.6cm, align=center},
    subterm/.style={draw, rounded corners, font=\small, fill=green!10, minimum width=1.6cm, align=center},
    >=Stealth
  ]
  % Original
  \node at (-4.5, 3.2) {\textbf{Before shatter}};
  \node[node] (lam) at (-4.5, 2.2) {$\lam$};
  \node[node] (fa) at (-5.5, 0.8) {$\forallE$};
  \node[node] (bv) at (-3.5, 0.8) {$\bvar(0)$};
  \node[node] (nat1) at (-6.3, -0.4) {$\Nat$};
  \node[node] (nat2) at (-4.7, -0.4) {$\Nat$};
  \draw[->] (lam) -- (fa);
  \draw[->] (lam) -- (bv);
  \draw[->] (fa) -- (nat1);
  \draw[->] (fa) -- (nat2);
  ...
  % Shattered
  \node at (1.5, 3.2) {\textbf{After shatter}};
  \node[node] (lam2) at (1.5, 2.2) {$\lam$};
  \node[hrefnode] (hr) at (0.5, 0.8) {$\hrefE(h_1)$};
  \node[node] (bv2) at (2.5, 0.8) {$\bvar(0)$};
  \draw[->] (lam2) -- (hr);
  \draw[->] (lam2) -- (bv2);

  % Store
  \node at (6.5, 3.2) {\textbf{SubtermStore}};
  \node[subterm] (s1) at (6.5, 1.8) {$h_1 \mapsto$};
  \node[node] (fa2) at (6.5, 0.6) {$\forallE$};
  \node[node] (nat3) at (5.7, -0.4) {$\Nat$};
  \node[node] (nat4) at (7.3, -0.4) {$\Nat$};
  \draw[->] (s1) -- (fa2);
  \draw[->] (fa2) -- (nat3);
  \draw[->] (fa2) -- (nat4);
\end{tikzpicture}
\caption{Subterm-level hash-consing.
  The identity function $\lambda(n{:}\Nat{\to}\Nat).\,n$ is shattered:
  the domain $\Nat \to \Nat$ (which exceeds the 33-byte threshold)
  is replaced by a hash reference $\hrefE(h_1)$ and stored separately.
  If another declaration shares the same subterm, it references the same $h_1$ entry---stored once globally.}
\label{fig:shatter}
\end{figure}
...
\begin{itemize}[nosep]
  \item \textbf{Integrity}: SHA-256 collision resistance ensures that distinct terms receive
...
\begin{enumerate}[nosep]
  ...
  \item the SHA-256 implementation being correct, and
...
Subterm-level hash-consing (Section~\ref{sec:subterm-hashing}) extends this model
to individual subterms, enabling fine-grained deduplication across the entire knowledge base.
...
Rocq agree on the core theory, though they diverge on several kernel-level details
...
We address the principal challenges.
...
\begin{itemize}[nosep]
  \item \textbf{Version prefix}: a future version of the format could prepend a version byte to
    every hash input, so that terms self-identify their hashing era.
    (The current tag bytes in Section~\ref{sec:hashing} are constructor tags, not version identifiers.)
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Subterm hash-consing}: shared subterms are stored once in the DHT and referenced
    by hash (Section~\ref{sec:subterm-hashing}), reducing both storage and network transfer.
...
The subterm-level hash-consing mechanism (Section~\ref{sec:subterm-hashing}) directly
addresses this: when a declaration is shattered for storage, shared subterms across
all declarations in the network are stored exactly once.
A type like $\Nat \to \Nat$ or a recursor application pattern that recurs in thousands
of proofs occupies a single DHT entry, referenced by hash everywhere else.
This transforms the storage cost from $O(n \cdot s)$ (where $n$ is the number of
declarations and $s$ is the average subterm size) to $O(n + s')$ (where $s'$ is the
number of \emph{unique} subterms), providing significant compression for large libraries
of tactic-elaborated proofs.
...
Unison~\cite{unison} is the closest precedent: it content-addresses general-purpose functional
programs using cryptographic hashes of abstract syntax trees.
Section~\ref{sec:unison} discusses the detailed comparison; the key differences are that
HashMath adds a type-to-inhabitants index, requires formal verification before storage,
and handles CIC-specific features (universe polymorphism, inductive types, strict positivity).

\subsection{Logipedia and Dedukti}

The Logipedia project~\cite{logipedia} and Dedukti~\cite{dedukti} translate proofs between
different type theories through the $\lambda\Pi$-calculus modulo theory.
HashMath takes a complementary approach, as discussed in Section~\ref{sec:interop}.
...
%% ============================================================
\section{Reference Implementation: Distribution Prototype}
\label{sec:prototype}

The reference implementation includes a working peer-to-peer distribution layer
built as a Rust sidecar process (\texttt{hm-net}) communicating with the Lean
\texttt{hm} CLI.
This section describes the architecture and protocol of this prototype,
which realizes the distribution model described in Section~\ref{sec:distribution}.

\subsection{Architecture}

The system follows a two-process architecture with \emph{separated discovery
and transfer}, analogous to BitTorrent's Mainline DHT and IPFS's Bitswap:
\begin{itemize}[nosep]
  \item \textbf{Lean process (\texttt{hm})}: handles parsing, elaboration,
    type checking, hashing, and serialization.
    All trust-critical operations remain in Lean.
  \item \textbf{Rust sidecar (\texttt{hm-net})}: runs a libp2p~\cite{libp2p}
    node combining Kademlia DHT~\cite{maymounkov2002kademlia} for
    \emph{provider discovery} (which peer has hash~$h$?)\ with a dedicated
    request-response protocol (\texttt{/hashmath/transfer/1.0.0}) for
    \emph{direct content transfer} between peers.
    The sidecar is entirely untrusted---it transports opaque bytes
    whose integrity is verified by the Lean process upon retrieval.
\end{itemize}

The key insight is that Kademlia is designed as a lookup-heavy, write-light
system.
Storing full records (up to 1~MB) in the DHT requires replicating to
$\sim$20~peers per write, limiting throughput to $\sim$5--20 completions/sec.
By storing only lightweight \emph{provider records} ($\sim$40~bytes each)
in the DHT and serving content directly, the system achieves much higher
throughput: publishing hundreds of declarations completes in a single batch
IPC call, with provider announcements propagating asynchronously at a
rate-limited 20/sec in the background.

The Lean process spawns \texttt{hm-net} as a child process and communicates
over stdin/stdout using a binary IPC protocol.
The sidecar's libp2p stack includes Kademlia for provider discovery, the
Identify protocol for peer metadata exchange, AutoNAT for NAT status detection,
and a Relay client for NAT traversal.

\subsection{IPC Protocol}

Communication uses length-prefixed binary frames:
\[
  \underbrace{4\text{-byte BE length}}_{L} \;\|\; \underbrace{\text{tag byte}}_{1} \;\|\; \underbrace{\text{payload}}_{L - 1}
\]

Six request types flow from Lean to Rust (tags \texttt{0x50}--\texttt{0x55}:
Ping, Publish, Fetch, GetPeers, Shutdown, PublishBatch) and seven response types
flow back (tags \texttt{0x60}--\texttt{0x66}: Pong, Published, Found, NotFound,
PeerList, Error, BatchPublished).
Hash values are transmitted as raw 32-byte SHA-256 digests;
variable-length fields use LEB128 length prefixes.

\subsection{Publish and Fetch}

\paragraph{Publishing.}
When the user runs \texttt{hm publish file.hm}, the Lean process type-checks
all declarations, then \emph{shatters} each declaration
(Section~\ref{sec:subterm-hashing}): large subterms are replaced by $\hrefE$
nodes and stored as separate entries.
All fragments (subterms and top-level declarations) are sent to the sidecar
in a single \texttt{PublishBatch} IPC call.
The sidecar stores each record locally (persisted to disk as a file keyed by
the hex-encoded hash) and returns immediately.
In the background, the sidecar announces provider records to the DHT via
Kademlia \texttt{START\_PROVIDING} at a rate-limited 20 announcements per
second, so the network gradually learns which peers hold which content.

\paragraph{Fetching.}
When the user runs \texttt{hm fetch <hash>}, the sidecar first checks its
local file store.
If the content is not local, it queries the DHT via \texttt{GET\_PROVIDERS}
to discover which peers hold the requested hash, then transfers the content
directly from a provider using the \texttt{/hashmath/transfer/1.0.0}
request-response protocol.
A fallback \texttt{GET\_RECORD} path provides backward compatibility with
older nodes that stored full records in the DHT.
The Lean process inspects the returned bytes for $\hrefE$ nodes (tag \texttt{0x19}),
fetches each referenced subterm, builds a \textsf{SubtermStore}, and
\emph{reassembles} the full declaration.
It then recomputes the SHA-256 hash to verify integrity, recursively fetches all
declaration-level dependencies (referenced hashes embedded in the term),
type-checks each declaration, and adds it to the environment.
The process is backward-compatible: declarations without $\hrefE$ nodes
(published before subterm hash-consing was added) are handled identically
to canonical-format records.
This ensures that \emph{no ill-typed term can enter the environment through
the network}---the sidecar is a transport layer only, and the Lean process
independently verifies every declaration.

\subsection{Persistent Storage}

The sidecar maintains a file-backed record store at a platform-specific
data directory.
Each Kademlia record is stored as a file named by the hex-encoded key under a
\texttt{records/} subdirectory.
An Ed25519 keypair is persisted at \texttt{keypair}, giving the node a stable
PeerId across restarts.

\subsection{Manifest-Based Bulk Sync}

For synchronizing entire libraries, HashMath supports \emph{manifest files}
(\texttt{.hmm}): plain-text files listing one \texttt{name hexhash} pair per line.
The \texttt{hm manifest --generate} command produces a manifest from source files,
and \texttt{hm fetch --manifest lib.hmm} fetches and verifies every entry.
This enables efficient bulk replication of curated libraries across nodes.

%% ============================================================
\section{Roadmap}
\label{sec:roadmap}

We outline a phased development plan, noting progress to date.

\paragraph{Phase 1: Core Infrastructure \textnormal{(complete)}.}
The CIC type checker, SHA-256 hash function, canonical binary serialization,
and content-addressed environment are implemented in Lean~4 with no external
dependencies.
The implementation includes over 90 automated tests: 34 test groups in the
internal test suite, 24 correct example files, and 32 files that must be
rejected---covering type checking, positivity, universe constraints, large
elimination, and error rejection.

\paragraph{Phase 2: Distribution Prototype \textnormal{(complete)}.}
A working peer-to-peer distribution layer is implemented using a Rust sidecar
with libp2p (Section~\ref{sec:prototype}).
The architecture separates discovery (Kademlia provider records) from content
transfer (direct peer-to-peer \texttt{/hashmath/transfer/1.0.0} protocol),
following the same pattern as BitTorrent and IPFS.
Batch IPC and background rate-limited provider announcements enable
high-throughput publishing.
The CLI supports publishing, fetching with recursive dependency resolution,
peer discovery, and manifest-based bulk sync.
Persistent file-backed storage, externalized bootstrap configuration, and
NAT traversal support are included.
Subterm-level hash-consing (Section~\ref{sec:subterm-hashing}) provides
global deduplication: declarations are shattered into subterm fragments
before publishing, enabling shared subterms across the entire network to be
stored exactly once.

\paragraph{Phase 3: Import Pipeline.}
...
\paragraph{Phase 4: AI Integration.}
...
\paragraph{Phase 5: Scaling and Ecosystem.}
Implement connection limits, rate limiting, and spam resistance.
...
\url{https://libp2p.io/}, 2024.

\bibitem{maymounkov2002kademlia}
P.~Maymounkov and D.~Mazi\`eres,
``Kademlia: A peer-to-peer information system based on the XOR metric,''
in \emph{Proc.\ IPTPS}, LNCS~2429, Springer, 2002, pp.~53--65.
