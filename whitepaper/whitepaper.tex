...
---              & Hash reference       & \checkmark & \texttt{href}: storage layer only ($\alpha{=}\bot$ blocks kernel use) \\
\hline
\end{tabular}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Structure expansion}: if $I$ is a single-constructor type with fields
        ...
        $c(\bar{p},\, \proj(h_I, 0, x),\, \ldots,\, \proj(h_I, n{-}1, x))$ where $x$
        is the major premise, enabling $\iota$-reduction to proceed.
\end{itemize}
...
Both $\hash(e)$ and $\hash(\textsf{reassemble}(\textsf{shatter}_\theta(e)))$ equal the same value,
because $\textsf{reassemble}$ recovers the original term exactly, and canonical serialization
is deterministic.
...
The system follows a two-process architecture:
\begin{itemize}[nosep]
  ...
  \item \textbf{Rust sidecar (\texttt{hm-net})}: runs a libp2p~\cite{libp2p}
    Kademlia DHT~\cite{maymounkov2002kademlia} node for peer-to-peer
    storage and retrieval of content-addressed records.
...
The sidecar's libp2p stack includes Kademlia for record storage, the Identify
protocol for peer discovery, AutoNAT for NAT status detection, and a Relay
client for NAT traversal.
...
Five request types flow from Lean to Rust (tags \texttt{0x50}--\texttt{0x54}:
Ping, Publish, Fetch, GetPeers, Shutdown) and six response types flow back
(tags \texttt{0x60}--\texttt{0x65}: Pong, Published, Found, NotFound, PeerList, Error).
...
For each declaration, the process first publishes all subterm entries (each keyed
by its content hash), then publishes the top-level declaration in the stored
wire format (containing $\hrefE$ pointers instead of duplicated subterms).
The sidecar stores each record locally (persisted to disk as a file keyed by
the hex-encoded hash) and replicates to the DHT via Kademlia \texttt{PUT\_RECORD}.

\paragraph{Fetching.}
When the user runs \texttt{hm fetch <hash>}, the sidecar first checks its
local file store, then queries the DHT via Kademlia \texttt{GET\_RECORD}.
