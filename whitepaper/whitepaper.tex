...
One new constructor, $\textsf{iref}$, is added for block-relative references in mutual
inductive definitions, for 9 constructors total.
...
$i \in \mathbb{N}$, and $k \in \mathbb{N}$ is a block-relative type index.
\end{definition}
...
\caption{Lean~4 \texttt{Expr} constructors and their status in HashMath.
  ...
  excluded (elaboration or syntactic sugar). One new constructor (\texttt{iref}) is added,
  for 9 total.}
...
expressions (\texttt{0x10}--\texttt{0x18}), declarations (\texttt{0x20}--\texttt{0x23}),
...
all declarations, serializes each to canonical binary form, and sends
\texttt{Publish} messages containing the hash and serialized bytes.
...
The Lean process deserializes the returned bytes, recomputes the SHA-256 hash
to verify integrity, recursively fetches all dependencies (referenced hashes
embedded in the term), type-checks each declaration, and adds it to the
environment.
