...
We briefly present the Calculus of Inductive Constructions (CIC), the type theory underlying
both Lean~4 and Rocq.
Our presentation follows Coquand and Huet~\cite{coquand1988}, Coquand and
Paulin-Mohring~\cite{coquand1990}, and Werner~\cite{werner1994}.

\subsection{Terms and Types}
\label{sec:terms}

CIC terms are defined by the following grammar, using de Bruijn indices for bound variables:
\[
  t, T \;\;::=\;\; x \;\mid\; c \;\mid\; \Sort(u) \;\mid\; t\;t \;\mid\;
  \lambda(x{:}T).\,t \;\mid\; \Pi(x{:}T).\,T \;\mid\; \mathsf{let}\; x{:}T := t \;\mathsf{in}\; t
\]
where:
\begin{itemize}[nosep]
  \item $x$ denotes a variable (represented internally as a de Bruijn index $i \in \mathbb{N}$),
  \item $c$ denotes a constant (a reference to a previously defined declaration),
  \item $\Sort(u)$ is a universe sort at level $u$,
  \item $t\;t$ is application, $\lambda(x{:}T).\,t$ is abstraction, $\Pi(x{:}T).\,T$ is the dependent product,
  \item $\mathsf{let}\; x{:}T := t \;\mathsf{in}\; t$ is a local definition.
\end{itemize}
Lean~4's kernel type \texttt{Expr} is a concrete realization of this grammar, with additional
fields for binder names and binder info (implicit/explicit annotations) that are relevant for
elaboration but not for the core type theory~\cite{lean4}.

\subsection{Typing Rules}
\label{sec:typing}

We present the key typing judgments.
A context $\Gamma$ is a list of typed variable declarations.
The judgment $\Gamma \vdash t : T$ asserts that term $t$ has type $T$ in context $\Gamma$.

\begin{mathpar}
  \inferrule*[Right=Sort]
    { }
    {\Gamma \vdash \Sort(i) : \Sort(i+1)}
  ...
  \inferrule*[Right=Pi]
    {\Gamma \vdash A : \Sort(i) \\ \Gamma, x{:}A \vdash B : \Sort(j)}
    {\Gamma \vdash \Pi(x{:}A).\,B : \Sort(\max(i,j))}
\end{mathpar}

\begin{mathpar}
  \inferrule*[Right=Lam]
    {\Gamma \vdash A : \Sort(i) \\ \Gamma, x{:}A \vdash t : B}
  ...
  \inferrule*[Right=Conv]
    {\Gamma \vdash t : T \\ T \equiv_\delta T'}
    {\Gamma \vdash t : T'}
\end{mathpar}

The conversion rule (Conv) allows replacing a type with a definitionally equal one.
We refer to Coquand and Paulin-Mohring~\cite{coquand1990} for the complete set of rules
including the let-binding rule, and to the Lean~4 kernel source for a modern implementation~\cite{lean4}.

\subsection{Reductions}
\label{sec:reductions}

Definitional equality $\equiv_\delta$ is generated by:
\begin{itemize}[nosep]
  \item \textbf{$\beta$-reduction}: $(\lambda(x{:}A).\,t)\;a \;\longrightarrow_\beta\; t[a/x]$
  \item \textbf{$\delta$-reduction}: unfolding a named constant to its definition
  \item \textbf{$\iota$-reduction}: reducing pattern matches on constructors
  \item \textbf{$\zeta$-reduction}: substituting let-bindings, $\mathsf{let}\;x{:}T := v\;\mathsf{in}\;t \;\longrightarrow_\zeta\; t[v/x]$
\end{itemize}
...
Formally, an inductive specification is a tuple $\textsf{Ind}\{p, \Gamma_I, \Gamma_C\}$ where
$p$ is the number of parameters, $\Gamma_I$ declares the type constructors, and $\Gamma_C$
declares the data constructors~\cite{paulin1993}.
The \emph{strict positivity} constraint ensures that recursive occurrences of the type being defined
appear only in strictly positive positions in constructor argument types, guaranteeing logical consistency.

\begin{example}[Natural numbers]
The natural numbers are defined inductively:
\[
  \textsf{Ind}\{0,\; (\Nat : \Type),\; (\textsf{zero} : \Nat,\; \textsf{succ} : \Nat \to \Nat)\}
\]
The elimination principle for $\Nat$ yields primitive recursion.
\end{example}

\begin{example}[Propositional equality]
The identity type $\Eq$ is an inductive family:
\[
  \textsf{Ind}\{1,\; (\Eq : \Pi(A{:}\Type).\,A \to A \to \Prop),\; (\textsf{refl} : \Pi(A{:}\Type).\,\Pi(a{:}A).\,\Eq\;A\;a\;a)\}
\]
\end{example}
...
\]
Universe constraints (e.g., $u \leq v$, $u + 1 \leq v$) are collected during typechecking
and must be satisfiable.
...
We define the hash function $\hash$ on CIC terms recursively.
All hashing uses SHA3-256~\cite{sha3}.
Tag strings ensure distinct term constructors never collide.

\begin{definition}[Term hash]
\label{def:hash}
The hash function $\hash : \textsf{Term} \to \{0,1\}^{256}$ is defined by:
\begin{align*}
  \hash(\bvar(i)) &= \SHA(\texttt{"bvar"} \concat \textsf{encode}(i)) \\
  \hash(\Sort(u)) &= \SHA(\texttt{"sort"} \concat \hash_{\textsf{level}}(u)) \\
  \hash(\const(n, \bar{u})) &= \SHA(\texttt{"const"} \concat \hash(\textsf{defn}(n)) \concat \hash_{\textsf{levels}}(\bar{u})) \\
  \hash(\app(f, a)) &= \SHA(\texttt{"app"} \concat \hash(f) \concat \hash(a)) \\
  \hash(\lam(\_, T, b, \_)) &= \SHA(\texttt{"lam"} \concat \hash(T) \concat \hash(b)) \\
  \hash(\forallE(\_, T, b, \_)) &= \SHA(\texttt{"forallE"} \concat \hash(T) \concat \hash(b)) \\
  \hash(\letE(\_, T, v, b, \_)) &= \SHA(\texttt{"letE"} \concat \hash(T) \concat \hash(v) \concat \hash(b))
\end{align*}
where $\textsf{defn}(n)$ resolves the constant $n$ to its full definition (type and value),
and $\_$ denotes fields excluded from the hash (binder names, binder info).
\end{definition}

Several design decisions are critical:

\paragraph{Binder names are excluded.}
The terms $\lambda(x{:}\Nat).\,x$ and $\lambda(y{:}\Nat).\,y$ are $\alpha$-equivalent
and receive the same hash.
Since we use de Bruijn indices internally, the binder name is already redundant---it is
carried only for pretty-printing.
Excluding it means $\alpha$-equivalence is built into the hash by construction.

\paragraph{Binder info is excluded.}
Lean~4 attaches \emph{binder info} to each binder: whether the argument is explicit, implicit,
strict implicit, or instance implicit.
This is an elaboration concern, not a type-theoretic one.
We exclude binder info from the core hash; elaboration metadata can be stored separately as
annotations on the hash.

\paragraph{Constants resolve to definitions.}
The hash of $\const(n, \bar{u})$ includes the hash of the \emph{definition} that $n$ refers to,
not the string $n$ itself.
...
\label{ex:identity}
Consider $\lambda(n{:}\Nat).\,n$, which in de Bruijn notation is $\lam(\_, \Nat, \bvar(0), \_)$.
Its hash is:
\[
  \hash(\lam(\_, \Nat, \bvar(0), \_)) = \SHA(\texttt{"lam"} \concat \hash(\Nat) \concat \SHA(\texttt{"bvar"} \concat \textsf{encode}(0)))
\]
where $\hash(\Nat)$ is the hash of the inductive definition of natural numbers.
\end{example}
...
Rocq's Gallina is also CIC.
The differences between Lean's and Rocq's kernel terms are largely syntactic: both have
the same term constructors, the same universe hierarchy, and the same inductive type scheme.
A Rocq export tool performs the analogous translation.
